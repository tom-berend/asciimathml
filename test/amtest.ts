...
export function test() {
    ...
    if (d) {
        ...
        */
        ...
        appnd(div, `overset x (=)`)
        appnd(div, `overset (x) =`)
        appnd(div, `overset(x+b)(=)`)  // <mover><mrow><mo>=</mo></mrow><mrow><mi>x</mi><mo>+</mo><mi>b</mi></mrow></mover>
        ...
        /*
         ...
         return
         // appnd(div, `bold ${font} int_0^1 f(x)dx `)
...
function fontTests(div: HTMLElement) {
    // [''/*, 'bb', 'sf', 'bbsf', 'bbb', 'cc', 'bbcc', 'tt', 'fr', 'bbfr', 'bbit',*/].forEach((font) => {
    let font = ''
    subtitle(div, font)
    appnd(div, `${font} abc`)
    appnd(div, `${font} (abc)`)
    appnd(div, `${font} bold(abc)`)

    appnd(div, `${font} "abc 01239 $%*"`)
    appnd(div, `bold ${font} "abc 01239 $%*"`)
    appnd(div, `bold bold ${font} "abc 01239 $%*"`)
    appnd(div, `${font} tan (x)`)
    appnd(div, `bold ${font} tan (x)`)
    appnd(div, `bold ${font} (tan (x))`)
    appnd(div, `${font} hat (x)`)
    appnd(div, `bold ${font} hat (x)`)
    appnd(div, `bold ${font} abs (x)`)
    appnd(div, `${font} sqrt log(x)`)
    appnd(div, `bold ${font} sqrt log(x)`)
    appnd(div, `bold ${font} (x_2)`)
    appnd(div, `bold ${font} x_2`)
    appnd(div, `${font} [[a,b,c,d]] `)
    appnd(div, `bold ${font}( [[a,b,c,d]])`)
    // appnd(div, `bold ${font} int_0^1 f(x)dx `)
...
function subtitle(div: HTMLElement, title: string) {
    ...
    td.style = 'background-color:aliceblue;max-width:35%;';
    tr.appendChild(td)
}
...
function appnd(div: HTMLElement, expr: string, comment: string = '') {
    ...
    tr.appendChild(td5)
    let lex = am.lexScanner(expr)
    // let rOut2 = JSON.stringify(rOut).replaceAll('],', ']<br/>')
    let rOut2 = JSON.stringify(lex).replaceAll('],', ']')
    td5.innerHTML = comment + rOut2

    let td6 = document.createElement('td')
    td6.style = 'max-width:25%;';
    tr.appendChild(td6)
    let pOut2 = pOut3.slice(135 + expr.length) // chop off the front
    ...
    // let inner = '<math  title="${str}">'
    // inner += `<mstyle mathcolor="blue" fontsize="1em" mathsize="1em" fontfamily="serif" mathvariant="serif" displaystyle="true">`


    // let pOut: [string, number] = ['', 0]

    // for (let i = 0; i < 10; i++) {

    //     if (pOut[1] < lex.length) {
    //         // if (i > 0) inner += '<mstyle mathcolor="red"><mi>.</mi></mstyle>'  // visual separator
    //         pOut = am.TSparseSexpr(lex, pOut[1], '', '')
    //         inner += pOut[0]
    //     }
    // }
    // td7.innerHTML += inner
